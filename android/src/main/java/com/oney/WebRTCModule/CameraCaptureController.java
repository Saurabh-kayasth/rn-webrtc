...
public class CameraCaptureController extends AbstractVideoCaptureController {
    ...
    /**
     * The {@link CameraVideoCapturer.CameraEventsHandler} used with
    ...
    public void zoomCamera(float zoomValue){
        Logging.d("NEW_ZOOM_VALUE",""+zoomValue);
        float maxZoom = 1.0f;
        try {
            Camera2Capturer cameraVideoCapturer = (Camera2Capturer) videoCapturer;
            maxZoom = cameraVideoCapturer.getMaxZoom();
            Logging.d("MAX_ZOOM",""+maxZoom);
        } catch (Exception e) {
            Log.e(TAG,e.getMessage());
        }

        float finalMaxZoom = maxZoom;
        executor.execute(() -> {
            try {
                changeZoomValue(1+zoomValue/10 - (10-finalMaxZoom)/10);
            } catch (Camera2Capturer.CameraException e) {
                e.printStackTrace();
            }
        });
    }

    private void changeZoomValue(float zoomValue) throws Camera2Capturer.CameraException {
        Camera2Capturer cameraVideoCapturer = (Camera2Capturer) videoCapturer;
